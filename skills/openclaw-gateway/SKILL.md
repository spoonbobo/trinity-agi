...
The gateway token is stored in `src/.env` as `OPENCLAW_GATEWAY_TOKEN`.
...
Config lives at `src/openclaw.json` (mounted read-only into the container at `~/.openclaw/openclaw.json`).
...
- The config file is NOT bind-mounted (atomic rename fails on Windows Docker bind mounts). It lives in the `openclaw-data` volume. To sync it back to the host: `docker cp trinity-openclaw:/home/node/.openclaw/openclaw.json src/openclaw.json`
...
**Config validation errors:** check `src/openclaw.json` syntax. The config is JSON5 but must pass OpenClaw's schema validation.
...
```
docker compose -f src/docker-compose.yml build --no-cache openclaw-gateway
docker compose -f src/docker-compose.yml up -d
```
